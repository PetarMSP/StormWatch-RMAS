...
class LocalReportRepository {
    ...
    suspend fun addReport(report: LocalWeatherReport) {
        ...
        val newReport = report.copy(userID = userId)

        reportsRef.add(newReport).await()
    }
    ...
    suspend fun like(reportId: String, userId: String?) {
        ...
        firestore.runTransaction { transaction ->
            ...
            if (!voteSnapshot.exists()) {
                ...
                transaction.update(reportDoc, "likes",
    ...
    suspend fun dislike(reportId: String, userId: String?) {
        ...
        firestore.runTransaction { transaction ->
            ...
            if (!voteSnapshot.exists()) {
                ...
                transaction.update(reportDoc, "dislikes",
                    com.google.firebase.firestore.FieldValue.increment(1))
            }
    ...
    fun getActiveReports(): Flow<List<LocalWeatherReport>> = callbackFlow {
        val listener = reportsRef
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                val reports = snapshot?.documents?.mapNotNull{doc -> doc.toObject(LocalWeatherReport::class.java)?.copy(id = doc.id)}?: emptyList()
                trySend(reports)
            }
